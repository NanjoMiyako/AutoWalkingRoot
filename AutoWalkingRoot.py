...
def main3():
    ...
    print(len(args))
    if len(args) != 7:
        print("コマンドライン引数は以下の6つを順に指定してください")
        print("1:ベース地図画像パス(拡張子付き)")
        print("2:道と交差点画像パス(拡張子付き)")
        print("3:初期X位置")
        print("4:初期Y位置")
        print("5:散歩最小距離")
        print("6:保存する散歩ルート画像ファイル名(拡張子付き)")
        return
    else:
        
        baseMapImgPath = args[1]
        RoadAndCrossImgPath = args[2]
        startX = int(args[3])
        startY = int(args[4])
        endDist = int(args[5])
        saveWalkRootFileName = args[6]
    ...
    baseImg = cv2.imread(baseMapImgPath)
    out_img = cv2.imread("blank3.bmp")
    
    #道と交差点が書かれた画像読み込み
    img1 = cv2.imread(RoadAndCrossImgPath)
    height, width, channel = img1.shape
    
    out_img = cv2.resize(out_img, (width, height))
    baseImg = cv2.resize(baseImg, (width, height))
    ...
    out_img2 = DrawImage(baseImg, out_img)
    cv2.imwrite(saveWalkRootFileName, out_img2)
...
def callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONUP:
        ...
        print("クリック位置は")
        str1 = "X座標:" + str(mouse_x)
        print(str1)
        str1 = "Y座標:" + str(mouse_y)
        print(str1)
        print("です")
